...
class ItemController extends Controller
{
    public function Index(Request $request) {
        ...
        if (!empty($search)) {
            // whereRaw() - metoda care ne permite inauntrul acesteia sa mai scriem anumite functii (strtolower...)
            // sau LOWER(item_name...) - deci nu lucreaza ca un simplu where ce asteapta anumite informatii specifice
            $items = Item::whereRaw('LOWER(item_name) LIKE ?', ['%' . strtolower
            // orWhereRaw() - metoda similara cu whereRaw doar ca este precedata de operatorul or
            // (vom avea afisat rezultatul atat de la primul whereRaw(...) cat si de la al doilea orWhereRaw(...))
            ($search) . '%']) -> orWhereRaw('LOWER(item_category) LIKE ?', ['%' . strtolower
            ($search) . '%']) -> get();
        } else {
    ...
    public function DeleteItem(Request $request) {
        ...
        return redirect("/") -> with("success", "Item deleted successfuly!");
    }
}
